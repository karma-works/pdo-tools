...
fields differ:
*   **V4**: first version, used by Designer 2.x
*   **V5**: second version, used by Designer 3.x series
*   **V6**: third version, used by Designer 4.x & 5.x series
...
*   Codepage for strings - Windows Codepage 932 for Japanese (also known as CP932, Shift-JIS), can be used even if codepage string is set to 'us-ascii' (see the cube.pdo sample).
    *   Examples of japanese strings:
        ...
        *   ＭＳ ゴシック - 'Ms Gothic' as font name (see [Wikipedia](http://en.wikipedia.org/wiki/List_of_CJK_fonts#Sans-serif))
*   Multi-byte values are stored in little-endian order.

### Notation

*   **element description format**: `size in B [data type] : description [possible values]`
*   **element grouping block**: `[name] { 1..N elements }`
*   **array of elements**: `array[number of elements] of [element name]`
...
**STRING**: Zero/null terminated string with specified length, 1 or 2 bytes per char (specified in FILE HEADER)
```
STRING {
  4B : length
  if (length > 0) {
...
**Notes:**
*   **MD5 hash**: The hashed value is generated as: `hash = MD5(string(edge_count + face_count) + password + string(TLCK))`
...
**VERTEX** (24B) - Vertex coordinates in 3D space
```
VERTEX {
  8B float : X
  8B float : Y
...
**2D VERTEX** (85B) - Vertex coordinates in 2D space
```
2D VERTEX {
...
*   PDO uses the texture in bottom row first order, therefore the V coordinate in FVERTEX gets flipped. Probably because windows bitmap works with this order too, and it was easier to flip a coordinate than the whole texture.
...
**UNKNOWN_ITEM** (No idea, doesn't seem to have any effect?)
```
UNKNOWN_ITEM {
{ 
